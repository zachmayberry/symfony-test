...
class MP3File
{
    ...
    public function getDuration($use_cbr_estimate=false)
    {
        ...
        while (!feof($fd))
        {
            ...
            else if ($block[0]=="\xff" && (ord($block[1])&0xe0) )
            {
                ...
                if (isset($info['Samples'], $info['Sampling Rate'])) { // FIX FOR NOTICE ERROR
    ...
    public static function parseFrameHeader($fourbytes)
    {
        ...
        // Lorenz Haase [Media Rocks]: fix division by zero error !!!
        if ($sample_rate !== 0) $info['Framesize'] = self::framesize($layer, $bitrate, $sample_rate, $padding_bit);
        if (isset($samples[$simple_version][$layer])) $info['Samples'] = $samples[$simple_version][$layer];
        return $info;
    }
